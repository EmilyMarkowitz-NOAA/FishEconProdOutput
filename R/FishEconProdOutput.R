...
EditCommData <- function(dat, category0) {
  ...
  temp0$AFS_NAME1 <-
    gsub(
      pattern = "\\*",
      replacement = "-",
      x = as.character(temp0$AFS_NAME1)
    )
  temp0$AFS_NAME1 <-
    gsub(
      pattern = "_",
  ...
  dol <- aggregate(
    ...
    by = list(
      year = temp0$Year,
      species = temp0$AFS_NAME1,
      category = temp0[, names(temp0) %in% category0]
    ),
    FUN = sum,
    na.rm = TRUE
  )
  ...
  for (i in 1:length(cat0)) {
    a <-
      data.frame(table(temp$category[temp$category %in% cat0[i]], temp$species[temp$category %in% cat0[i]]))
    a <- a[!(a$Freq %in% 0), ]
    if (nrow(a) < 2) {
  ...
  temp$species <-
    paste0(numbers0(as.numeric(factor(temp$category))),
           "_",
           numbers0(x = as.numeric(factor(temp$species))),
           temp$species)
  ...
  for (i in 1:length(cat0)) {
    spp.list[[i]] <-
      as.character(unique(dat$AFS.Name[dat[, names(dat) %in% category0] %in% cat0[i]]))
    tsn.list[[i]] <-
      as.character(unique(dat$Tsn[dat[, names(dat) %in% category0] %in% cat0[i]]))
  ...
  names(temp.q)[names(temp.q) %in% "temp"] <-
    paste0("QE",
           numbers0(x = c(0, length(cat0)))[1],
           "_",
           paste(rep_len(
             x = 0,
             length.out = nchar(numbers0(x = as.numeric(
               factor(temp$species)
             ))[1])
           ), collapse = ""),
           "Total")
  for (i in 1:length(cat)) {
    if (sum(grepl(x = names(temp.q), pattern = paste0("Q", cat[i], "_"))) == 1) {
      temp.q$temp <-
        temp.q[, grepl(x = names(temp.q),
                       pattern = paste0("Q", cat[i], "_"))]
    } else {
      temp.q$temp <-
        rowSums(temp.q[, grepl(x = names(temp.q),
                               pattern = paste0("Q", cat[i], "_"))], na.rm = T)
    }
    names(temp.q)[names(temp.q) %in% "temp"] <-
      paste0(
        "QE",
  ...
  temp.v$temp <- temp00
  names(temp.v)[names(temp.v) %in% "temp"] <-
    paste0("VE",
           numbers0(x = c(0, length(cat0)))[1],
           "_",
           paste(rep_len(
  ...
  for (i in 1:length(cat)) {
    if (sum(grepl(x = names(temp.v), pattern = paste0("V", cat[i], "_"))) == 1) {
      temp.v$temp <-
        temp.v[, grepl(x = names(temp.v),
                       pattern = paste0("V", cat[i], "_"))]
    } else {
      temp.v$temp <-
        rowSums(temp.v[, grepl(x = names(temp.v),
                               pattern = paste0("V", cat[i], "_"))], na.rm = T)
    }
    names(temp.v)[names(temp.v) %in% "temp"] <-
      paste0(
        "VE",
...
PriceMethodOutput_Category <-
  function(temp,
    ...
    if (sum(minyr:maxyr %in% unique(temp.cat$Year)) != length(minyr:maxyr)) {
      ...
      temp0$Year <-
        setdiff(x = minyr:maxyr, y = unique(temp.cat$Year))
      temp0$cat <- category
      temp0$time <-
        setdiff(x = ((minyr:maxyr) - (minyr - 1)), y = unique(temp.cat$time))
      temp0$prod <- 0
