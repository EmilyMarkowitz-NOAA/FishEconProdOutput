...
#' Takes a string of words and combines them into a sentance that lists them.
#'
#' This function alows you to automatically edit data into the correct format from the NOAA Fisheries One Stop Shop https://foss.nmfs.noaa.gov/apexfoss/f?p=215:200:::::: plus a column with how you want to categorize your data.
#' @param dat data from the NOAA Fisheries One Stop Shop https://foss.nmfs.noaa.gov/apexfoss/f?p=215:200::::::
#' @param category0 string. Name of a column that contains the category names.
#' @keywords Edit Commercial Data
#' @export
#' @examples
#' EditCommData()
...
#' Caluclate the price change.
#'
#' This function allows you to caluclate the price change.
#' @param R0 Names of columns with revenue share data.
#' @param P0 Names of columns with price or price index data.
#' @keywords Price, Change, Price Index, Revenue share, Revenue-share
#' @export
#' @examples
#' PriceChange()
...
#' Caluclate the price Index
#'
#' This function allows you to caluclate the price change.
#' @param dat Default data.
#' @param BaseColName The base names (the leading category name) of columns with price or price index data.
...
#' @param var Vairable you want to find the price index from. Default = "PC". These characters preceed your data column name 'BaseColName'
#' @keywords Price, Change, Price Index, Revenue share, Revenue-share
#' @export
#' @examples
#' PriceIndex()
PriceIndex <- function(dat, BaseColName, baseyr, var = "PC") {
  ...
  tempPI_yr1<-c(1, rep_len(x = NA, length.out = nrow(dat)-1))

  PC0<-dat[,names(dat) %in% paste0(var, BaseColName)] #this is equal to ln(P_it/P_it-1)
...
#'
#' This function provides a general mental check - how many of your individual data are significantly increaseing, decreasing, or not changing? This will help the user understand the results they get from their analysis.
#' @param Columns Names of columns to apply this action to.
#' @param temp Name of dataset to apply this action to.
#' @export
#' @examples
#' lmCheck()
...
#' Quantity Methods - Category Level
#'
#' This function systematically runs the Quantity Method Productivity Output analysis for all species of a cateorgy.
#' @param temp Default dataset.
#' @param ii Category number.
#' @param maxyr The maxium year to assess in the dataset.
#' @param minyr The minium year to assess in the dataset.
...
QuantityMethodOutput_Category<-#ImplicitQuantityOutput.speciescat.q<-
...
#' Another method for calculating TFP
#'
#' Another method for calculating TFP
#' @param temp.output Dataset from productivity output.
#' @param temp.input Dataset from productivity input.
...
xunits<-function(val, combine=T) {
  ...
  if (sigfig0<=5) {
    # if (sigfig0<4) {
    unit<-""
    x<-format(x = val, big.mark = ",", digits = 0, scientific = F)
