...
tsn.id$TSN<-as.numeric(tsn.id$TSN)
...
aa<-list.files(path = paste0(dir_analyses), 
               pattern = paste0(minyr, "To", maxyr, "_NorE_category"), 
               full.names = TRUE)
...
temp<-read.xlsx(bb[grep(pattern = "_AllData", x = bb)], reg_order[1])

temp <- temp %>% 
  select(c(cat, Year, v, PI_CB, Q_CB, QI_CB)) 

knitr::kable(head(temp), booktabs = T) %>%
  kable_styling(latex_options = "striped")
```
...
figures.list$Northeast__PI_CB_PI

figures.list$Northeast__Q_CB_CatTot_QCatTot
